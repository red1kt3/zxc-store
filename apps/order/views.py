        ...
        if form.is_valid():
            with transaction.atomic():
                ...
                for row in cart['cart']:
                    ...
                    Product.objects.filter(id=row.product.id).update(quantity=row.product.quantity - check_quantity(row.quantity, row.product))
...
@login_required
def remove_from_cart(request, row_id):
